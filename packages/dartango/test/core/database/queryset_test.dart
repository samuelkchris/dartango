...
class TestUser extends Model {
  ...
  TestUser.fromMap(Map<String, dynamic> data) : super.fromMap(data);

  @override
  ModelMeta get meta => const ModelMeta(tableName: 'test_users');
...
void main() {
  ...
  tearDownAll(() async {
    ...
    try {
      final file = File('test_queryset.db');
  ...
  group('QuerySet Basic Operations', () {
    test('should retrieve all records', () async {
      final queryset = QuerySet<TestUser>(
          TestUser, 'test_users', 'test', (data) => TestUser.fromMap(data));
  ...
  group('QuerySet Filtering', () {
    ...
    test('should filter with lookup types', () async {
      ...
      final containsUsers =
          await queryset.filter({'name__contains': 'Johnson'}).all();
      expect(containsUsers.length, equals(1));
      expect(containsUsers[0].name, equals('Alice Johnson'));

      // Test startswith lookup
      final startsWithUsers =
          await queryset.filter({'name__startswith': 'Bob'}).all();
  ...
  group('QuerySet Get Operations', () {
    ...
    test('should throw DoesNotExistException for missing record', () async {
      ...
      expect(() => queryset.get({'email': 'nonexistent@example.com'}),
          throwsA(isA<DoesNotExistException>()));
    });

    test('should throw MultipleObjectsReturnedException for multiple records',
        () async {
      final queryset = QuerySet<TestUser>(
          TestUser, 'test_users', 'test', (data) => TestUser.fromMap(data));

      expect(() => queryset.get({'is_active': true}),
    ...
    test('should return null for getOrNull with missing record', () async {
      final queryset = QuerySet<TestUser>(
          TestUser, 'test_users', 'test', (data) => TestUser.fromMap(data));
      final user =
          await queryset.getOrNull({'email': 'nonexistent@example.com'});
  ...
  group('QuerySet Modification Operations', () {
    test('should update records', () async {
      final queryset = QuerySet<TestUser>(
          TestUser, 'test_users', 'test', (data) => TestUser.fromMap(data));
      final affectedRows =
          await queryset.filter({'name': 'Alice Johnson'}).update({'age': 26});
    ...
    test('should delete records', () async {
      ...
      final deletedRows =
          await queryset.filter({'email': 'delete@example.com'}).delete();
  ...
  group('QuerySet Chaining', () {
    ...
    test('should create independent clones', () async {
      final baseQueryset = QuerySet<TestUser>(
          TestUser, 'test_users', 'test', (data) => TestUser.fromMap(data));
  ...
  group('QuerySet Distinct Operations', () {
    test('should apply distinct to query', () async {
      ...
      final queryset = QuerySet<TestUser>(
          TestUser, 'test_users', 'test', (data) => TestUser.fromMap(data));
      final distinctUsers =
          await queryset.distinct().values(['age']).orderBy(['age']).all();
