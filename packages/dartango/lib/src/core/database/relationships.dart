...
class ForeignKey<T extends Model> extends Field<T> {
  ...
  void validate(T? value) {
    ...
    if (value != null && value.pk == null) {
      throw ValidationException('Related object must be saved before assignment');
    }
  }
...
class ManyToManyManager<T extends Model> {
  ...
  QuerySet<T> all() {
    ...
    final query = QuerySet<T>(relatedModel, relatedTable)
        .extra(
          tables: [throughTable],
          where: '$throughTable.$sourceColumn = ? AND $throughTable.$targetColumn = $relatedTable.id',
          params: [instance.pk],
        );
    return query;
  }
  ...
  Future<void> add(T object) async {
    ...
    try {
      final builder = InsertQueryBuilder(throughTable)
          .values({
  ...
  Future<void> remove(T object) async {
    ...
    try {
      final builder = DeleteQueryBuilder(throughTable)
          .where('$sourceColumn = ? AND $targetColumn = ?', [instance.pk, object.pk]);
  ...
  Future<List<T>> bulkCreate(List<Map<String, dynamic>> dataList) async {
    ...
    final connection = await DatabaseRouter.getConnection(instance.database);
    try {
      final insertData = objects.map((obj) => {
        sourceColumn: instance.pk,
        targetColumn: obj.pk,
      }).toList();
...
class ReverseRelationshipManager<T extends Model> {
  ...
  Future<List<T>> bulkCreate(List<Map<String, dynamic>> dataList) async {
    ...
    final objects = dataList.map((data) => (relatedModel as dynamic).fromMap(data) as T).toList();
    return await Model.bulkCreate(objects.cast<Model>()).then((models) => models.cast<T>());
  }
...
Map<String, RelationshipDescriptor> extractRelationships(Model model) {
  ...
  for (final entry in model._fields.entries) {
    ...
    if (field is ForeignKey || field is OneToOneField || field is ManyToManyField) {
      relationships[fieldName] = RelationshipDescriptor(field as dynamic, fieldName);
    }
  }
...
extension RelationshipExtensions on Model {
  Map<String, RelationshipDescriptor> get relationships => extractRelationships(this);
