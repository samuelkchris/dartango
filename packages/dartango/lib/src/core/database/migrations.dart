...
class SchemaEditor {
  ...
  Future<void> createModel(Type modelType, Map<String, Field> fields, ModelMeta meta) async {
  ...
  Future<void> addField(Type modelType, String fieldName, Field field, bool preserveDefault) async {
    final tableName = Model.getTableName(modelType);
    ...
    if (field.indexed) {
      await createIndex(modelType, '${tableName}_${fieldName}_idx', [fieldName], field.unique);
    }
  }
  ...
  Future<void> alterField(Type modelType, String fieldName, Field oldField, Field newField) async {
    final tableName = Model.getTableName(modelType);
    
    if (databaseType == 'sqlite') {
      await _recreateTableWithAlteredColumn(tableName, fieldName, oldField, newField);
    } else {
  ...
  Future<void> renameField(Type modelType, String oldName, String newName) async {
    final tableName = Model.getTableName(modelType);
  ...
  Future<void> createIndex(Type modelType, String indexName, List<String> fields, bool unique, [String? condition]) async {
    final tableName = Model.getTableName(modelType);
    final uniqueStr = unique ? 'UNIQUE ' : '';
    final conditionStr = condition != null ? ' WHERE $condition' : '';
    final sql = 'CREATE ${uniqueStr}INDEX $indexName ON $tableName (${fields.join(', ')})$conditionStr';
    await connection.execute(sql, []);
  }
  ...
  List<String> _buildConstraints(Map<String, Field> fields, ModelMeta meta) {
    ...
    for (final entry in fields.entries) {
      ...
      if (field is ForeignKey) {
        ...
        constraints.add('FOREIGN KEY ($fieldName) REFERENCES $referencedTable($referencedColumn) ON DELETE ${field.onDelete} ON UPDATE ${field.onUpdate}');
      }
    }
  ...
  Future<void> _createManyToManyTable(ManyToManyField field, String sourceTable) async {
  ...
  Future<void> _createIndexes(String tableName, Map<String, Field> fields, ModelMeta meta) async {
    for (final entry in fields.entries) {
      final fieldName = entry.key;
      final field = entry.value;
  ...
  Future<void> _recreateTableWithoutColumn(String tableName, String columnName) async {
    final tempTableName = '${tableName}_temp';
    
    final existingColumns = await _getTableColumns(tableName);
    final newColumns = existingColumns.where((col) => col != columnName).toList();
    
    final createTempSql = 'CREATE TABLE $tempTableName AS SELECT ${newColumns.join(', ')} FROM $tableName';
    await connection.execute(createTempSql, []);
    
    await connection.execute('DROP TABLE $tableName', []);
    await connection.execute('ALTER TABLE $tempTableName RENAME TO $tableName', []);
  }
  
  Future<void> _recreateTableWithAlteredColumn(String tableName, String columnName, Field oldField, Field newField) async {
    final tempTableName = '${tableName}_temp';
    
    await _getTableColumns(tableName);
    
    final createTempSql = 'CREATE TABLE $tempTableName AS SELECT * FROM $tableName';
    await connection.execute(createTempSql, []);
    
    await connection.execute('DROP TABLE $tableName', []);
    
    final newColumnDef = _buildColumnDefinition(columnName, newField);
    await connection.execute('ALTER TABLE $tempTableName ALTER COLUMN $newColumnDef', []);
    
    await connection.execute('ALTER TABLE $tempTableName RENAME TO $tableName', []);
  }
  
  Future<void> _recreateTableWithRenamedColumn(String tableName, String oldName, String newName) async {
    final tempTableName = '${tableName}_temp';
    
    final existingColumns = await _getTableColumns(tableName);
    final newColumns = existingColumns.map((col) => col == oldName ? '$col AS $newName' : col).toList();
...
class MigrationRecorder {
  ...
  Future<void> recordApplied(String app, String name) async {
    ...
    final sql = 'INSERT OR REPLACE INTO dartango_migrations (app, name, applied) VALUES (?, ?, ?)';
    await connection.execute(sql, [app, name, DateTime.now().toIso8601String()]);
  }
  ...
  Future<List<String>> getAppliedMigrations(String app) async {
    await ensureMigrationTable();
    final sql = 'SELECT name FROM dartango_migrations WHERE app = ? ORDER BY applied';
  ...
  Future<bool> isMigrationApplied(String app, String name) async {
    await ensureMigrationTable();
    final sql = 'SELECT COUNT(*) as count FROM dartango_migrations WHERE app = ? AND name = ?';
...
class MigrationExecutor {
  ...
  Future<void> migrate(String app, List<Migration> migrations, {String? targetMigration}) async {
  ...
  Future<void> rollback(String app, List<Migration> migrations, String targetMigration) async {
    final appliedMigrations = await recorder.getAppliedMigrations(app);
    final editor = SchemaEditor(connection, databaseType);
  ...
  Future<void> _applyMigration(String app, Migration migration, SchemaEditor editor) async {
  ...
  Future<void> _rollbackMigration(String app, Migration migration, SchemaEditor editor) async {
    ...
    for (final entry in targetState.entries) {
      ...
      if (!currentState.containsKey(modelType)) {
        migrations.add(CreateModelMigration(
          ...
          name: '${DateTime.now().millisecondsSinceEpoch}_create_${modelType.toString().toLowerCase()}',
        ));
      } else {
    ...
    for (final entry in currentState.entries) {
      ...
      if (!targetState.containsKey(modelType)) {
        migrations.add(DeleteModelMigration(
          ...
          name: '${DateTime.now().millisecondsSinceEpoch}_delete_${modelType.toString().toLowerCase()}',
        ));
      }
    }
    ...
    for (final entry in targetFields.entries) {
      ...
      if (!currentFields.containsKey(fieldName)) {
        migrations.add(AddFieldMigration(
          ...
          fieldName: fieldName,
          field: targetField,
          name: '${DateTime.now().millisecondsSinceEpoch}_add_${modelType.toString().toLowerCase()}_$fieldName',
        ));
      } else {
        ...
        if (_fieldsAreDifferent(currentField, targetField)) {
          migrations.add(AlterFieldMigration(
            ...
            name: '${DateTime.now().millisecondsSinceEpoch}_alter_${modelType.toString().toLowerCase()}_$fieldName',
          ));
        }
      }
    }
    ...
    for (final entry in currentFields.entries) {
      ...
      if (!targetFields.containsKey(fieldName)) {
        migrations.add(RemoveFieldMigration(
          ...
          fieldName: fieldName,
          field: currentField,
          name: '${DateTime.now().millisecondsSinceEpoch}_remove_${modelType.toString().toLowerCase()}_$fieldName',
        ));
      }
    }
