...
class GlobalSettings {
  ...
  static BaseSettings get instance {
    if (_instance == null) {
      throw Exception(
          'Settings not configured. Call GlobalSettings.configure() first.');
    }
    return _instance!;
  }
  ...
  static List<String> get allowedHosts =>
      getSetting('ALLOWED_HOSTS', <String>['*']);
  static bool get hasAllowedHosts =>
      allowedHosts.isNotEmpty && !allowedHosts.contains('*');
  ...
  static List<String> get installedApps =>
      getSetting('INSTALLED_APPS', <String>[]);
  ...
  static Map<String, dynamic> get databases =>
      getSetting('DATABASES', <String, dynamic>{});
  ...
  static List<String> get staticFilesDirs =>
      getSetting('STATICFILES_DIRS', <String>[]);
  static List<String> get staticFilesFinders =>
      getSetting('STATICFILES_FINDERS', <String>[]);

  static List<Map<String, dynamic>> get templates =>
      getSetting('TEMPLATES', <Map<String, dynamic>>[]);
  static Map<String, dynamic> get defaultTemplate =>
      templates.isNotEmpty ? templates.first : {};

  static String get defaultFromEmail =>
      getSetting('DEFAULT_FROM_EMAIL', 'webmaster@localhost');
  ...
  static String get sessionCookieName =>
      getSetting('SESSION_COOKIE_NAME', 'sessionid');
  static String get sessionCookieAge =>
      getSetting('SESSION_COOKIE_AGE', '1209600'); // 2 weeks
  static String get sessionCookieDomain =>
      getSetting('SESSION_COOKIE_DOMAIN', '');
  static String get sessionCookiePath => getSetting('SESSION_COOKIE_PATH', '/');
  static bool get sessionCookieSecure =>
      getSetting('SESSION_COOKIE_SECURE', false);
  static bool get sessionCookieHttponly =>
      getSetting('SESSION_COOKIE_HTTPONLY', true);
  static String get sessionCookieSamesite =>
      getSetting('SESSION_COOKIE_SAMESITE', 'Lax');
  static bool get sessionSaveEveryRequest =>
      getSetting('SESSION_SAVE_EVERY_REQUEST', false);
  static bool get sessionExpireAtBrowserClose =>
      getSetting('SESSION_EXPIRE_AT_BROWSER_CLOSE', false);
  static String get sessionEngine =>
      getSetting('SESSION_ENGINE', 'django.contrib.sessions.backends.db');
  static String get sessionFileStoragePath =>
      getSetting('SESSION_FILE_STORAGE_PATH', '');

  static String get csrfCookieName =>
      getSetting('CSRF_COOKIE_NAME', 'csrftoken');
  static String get csrfCookieAge =>
      getSetting('CSRF_COOKIE_AGE', '31449600'); // 1 year
  ...
  static bool get csrfCookieHttponly =>
      getSetting('CSRF_COOKIE_HTTPONLY', false);
  static String get csrfCookieSamesite =>
      getSetting('CSRF_COOKIE_SAMESITE', 'Lax');
  static String get csrfHeaderName =>
      getSetting('CSRF_HEADER_NAME', 'HTTP_X_CSRFTOKEN');
  static List<String> get csrfTrustedOrigins =>
      getSetting('CSRF_TRUSTED_ORIGINS', <String>[]);

  static Map<String, dynamic> get caches =>
      getSetting('CACHES', <String, dynamic>{});
  static Map<String, dynamic> get defaultCache => caches['default'] ?? {};

  static Map<String, dynamic> get logging =>
      getSetting('LOGGING', <String, dynamic>{});
  ...
  static List<String> get authenticationBackends =>
      getSetting('AUTHENTICATION_BACKENDS', <String>[]);
  static String get loginUrl => getSetting('LOGIN_URL', '/accounts/login/');
  static String get loginRedirectUrl =>
      getSetting('LOGIN_REDIRECT_URL', '/accounts/profile/');
  ...
  static bool get disallowedUserAgents =>
      getSetting('DISALLOWED_USER_AGENTS', false);

  static int get dataUploadMaxMemorySize =>
      getSetting('DATA_UPLOAD_MAX_MEMORY_SIZE', 2621440); // 2.5MB
  static int get dataUploadMaxNumberFields =>
      getSetting('DATA_UPLOAD_MAX_NUMBER_FIELDS', 1000);
  static int get fileUploadMaxMemorySize =>
      getSetting('FILE_UPLOAD_MAX_MEMORY_SIZE', 2621440); // 2.5MB
  static List<String> get fileUploadHandlers =>
      getSetting('FILE_UPLOAD_HANDLERS', <String>[]);
  static String get fileUploadTempDir =>
      getSetting('FILE_UPLOAD_TEMP_DIR', Directory.systemTemp.path);
  static int get fileUploadPermissions =>
      getSetting('FILE_UPLOAD_PERMISSIONS', 420); // 0o644
  static int get fileUploadDirectoryPermissions =>
      getSetting('FILE_UPLOAD_DIRECTORY_PERMISSIONS', 493); // 0o755

  static List<String> get secureProxyHeaders =>
      getSetting('SECURE_PROXY_HEADERS', <String>[]);
  static bool get secureRedirectExempt =>
      getSetting('SECURE_REDIRECT_EXEMPT', false);
  ...
  static bool get secureHstsIncludeSubdomains =>
      getSetting('SECURE_HSTS_INCLUDE_SUBDOMAINS', false);
  static bool get secureHstsPreload => getSetting('SECURE_HSTS_PRELOAD', false);
  static bool get secureContentTypeNosniff =>
      getSetting('SECURE_CONTENT_TYPE_NOSNIFF', true);
  static bool get secureBrowserXssFilter =>
      getSetting('SECURE_BROWSER_XSS_FILTER', true);
  static String get secureReferrerPolicy =>
      getSetting('SECURE_REFERRER_POLICY', 'same-origin');
  ...
  static int get serverMaxRequestsJitter =>
      getSetting('SERVER_MAX_REQUESTS_JITTER', 0);
  static int get serverTimeout => getSetting('SERVER_TIMEOUT', 30);
  static int get serverKeepAlive => getSetting('SERVER_KEEP_ALIVE', 2);
  static int get serverMaxConnectionsPerChild =>
      getSetting('SERVER_MAX_CONNECTIONS_PER_CHILD', 0);
  ...
  static void validate() {
    ...
    if (isDebug && hasAllowedHosts) {
      _logger.warning(
          'DEBUG is True but ALLOWED_HOSTS is not set to allow all hosts');
    }
