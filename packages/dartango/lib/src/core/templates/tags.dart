  ...
  TemplateNode parse(TemplateParser parser, Token token, List<String> args) {
    if (args.length < 3 || args[1] != 'in') {
      throw TemplateSyntaxException(
          'For tag requires format: for item in items');
    }
...
class WithTag extends TemplateTag {
  ...
  TemplateNode parse(TemplateParser parser, Token token, List<String> args) {
    if (args.length < 3 || args[1] != 'as') {
      throw TemplateSyntaxException(
          'With tag requires format: with expression as variable');
    }
...
class RegroupTag extends TemplateTag {
  ...
  TemplateNode parse(TemplateParser parser, Token token, List<String> args) {
    if (args.length < 5 || args[1] != 'by' || args[3] != 'as') {
      throw TemplateSyntaxException(
          'Regroup tag requires format: regroup list by attribute as variable');
    }
...
class CycleTag extends TemplateTag {
  ...
  TemplateNode parse(TemplateParser parser, Token token, List<String> args) {
    ...
    final values =
        args.map((arg) => arg.replaceAll('"', '').replaceAll("'", '')).toList();
...
class FirstOfTag extends TemplateTag {
  ...
  TemplateNode parse(TemplateParser parser, Token token, List<String> args) {
    if (args.isEmpty) {
      throw TemplateSyntaxException(
          'Firstof tag requires at least one variable');
    }
...
class WidthRatioTag extends TemplateTag {
  ...
  TemplateNode parse(TemplateParser parser, Token token, List<String> args) {
    if (args.length < 3) {
      throw TemplateSyntaxException(
          'Widthratio tag requires format: widthratio current max scale');
    }
