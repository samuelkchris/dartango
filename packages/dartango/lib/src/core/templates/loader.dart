  ...
  TemplateSource loadTemplate(String name) {
    for (final templateDir in templateDirs) {
      ...
      if (file.existsSync()) {
        ...
        final content =
            file.readAsStringSync(encoding: Encoding.getByName(encoding)!);
    ...
    throw TemplateNotFoundException(
        'Template "$name" not found in any of the template directories');
  }
...
class AssetLoader extends TemplateLoader {
  ...
  @override
  TemplateSource loadTemplate(String name) {
    final fullPath = package.isEmpty
        ? '$assetPath/$name'
        : 'packages/$package/$assetPath/$name';
...
class MemoryLoader extends TemplateLoader {
  ...
  TemplateSource loadTemplate(String name) {
    if (!_templates.containsKey(name)) {
      throw TemplateNotFoundException(
          'Template "$name" not found in MemoryLoader');
    }
...
class CompoundLoader extends TemplateLoader {
  ...
  TemplateSource loadTemplate(String name) {
    ...
    throw TemplateNotFoundException(
        'Template "$name" not found in any compound loader');
  }
